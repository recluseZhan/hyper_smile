...
int calc_md5(char*filename,char*dest)
{
    ...
    while(1)
    {
        read_len = vfs_read(fp, hexbuf,sizeof(hexbuf),&pos); 
        pos = pos + sizeof(hexbuf);
...
{
    printk(KERN_ALERT"md5 module is entering..\n");
...
static void __exit md5_exit(void){
    printk(KERN_ALERT"md5 module is leaving..\n");
}
//EXPORT_SYMBOL(calc_md5);
