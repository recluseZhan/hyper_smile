...
//
unsigned long get_host_phys_addr(unsigned long gpa,unsigned long eptp){
    unsigned long pml4_index,pdpt_index,pd_index,pt_index;
    unsigned long *pml4,*pdpt,*pd,*pt;
    unsigned long *vpml4,*vpdpt,*vpd,*vpt;
    unsigned long pml4_entry,pdpt_entry,pd_entry,pt_entry;
    unsigned long size = 8;

   // unsigned long pml4_value;
    //,pdpt_entry,pd_entry,pt_entry;
    //uint64_t hpa;
    pml4_index=(gpa>>39)&0x1ff;
    pdpt_index=(gpa>>30)&0x1ff;
    pd_index=(gpa>>21)&0x1ff;
    pt_index=(gpa>>12)&0x1ff;
    printk("pml4i=%lx,pdpti=%lx,pdi=%lx,pti=%lx\n",(unsigned long)pml4_index,(unsigned long)pdpt_index,(unsigned long)pd_index,(unsigned long)pt_index);
    
    
    
    pml4=(unsigned long *)((eptp&0xfffffffffffff000)+pml4_index*size);
    vpml4 = (unsigned long *)phys_to_virt((phys_addr_t)pml4);
    pml4_entry = *vpml4;
    
    pdpt=(unsigned long *)((pml4_entry&0xfffffffffffff000)+pdpt_index*size);
    vpdpt = (unsigned long *)phys_to_virt((phys_addr_t)pdpt);
    pdpt_entry = *vpdpt;
    
    pd = (unsigned long *)((pdpt_entry&0xfffffffffffff000)+pd_index*size);
    vpd = (unsigned long *)phys_to_virt((phys_addr_t)pd);
    pd_entry = *vpd;
    
    pt = (unsigned long *)((pd_entry&0xfffffffffffff000)+pt_index*size);
    vpt = (unsigned long *)phys_to_virt((phys_addr_t)pt);
    pt_entry = (*vpt)&0xfffffffffffffff0; 
    
    printk("pml4e=%lx,pdpte=%lx,pde=%lx,pte=%lx\n",(unsigned long)pml4_entry,(unsigned long)pdpt_entry,(unsigned long)pd_entry,(unsigned long)pt_entry);
    /*
    pml4_entry=pml4[pml4_index];
    printk("pml4_entry=%lx",(unsigned long)pml4_entry);
    
    pdpt=(uint64_t *)(pml4_entry&0xfffffffffffff000);
    if((pdpt[pdpt_index]&0x1)==0){
        printk("pdpt null");
        return 0;
    }
    pdpt_entry=pdpt[pdpt_index];
    printk("pdpt_entry=%lx",(unsigned long)pdpt_entry);
    
    pd=(uint64_t *)(pdpt_entry&0xfffffffffffff000);
    if((pd[pd_index]&0x1)==0){
        printk("pd null");
        return 0;
    }
    pd_entry=pd[pd_index];
    printk("pd_entry=%lx",(unsigned long)pd_entry);
    
    pt=(uint64_t *)(pd_entry&0xfffffffffffff000);
    if((pt[pt_index]&0x1)==0){
        printk("pt null");
        return 0;
    }
    pt_entry=pt[pt_index];
    printk("pt_entry=%lx",(unsigned long)pt_entry);
    
    hpa=(pt_entry&0xfffffffffffff000)|(gpa&0xfff);
    return hpa;
    */
    return 0;
}
...
*/	
	...
	int op_64_bit;
	//struct page *page=allocate_page(tra_kvm);
	//unsigned long page_add;
	//page_add=(unsigned long)page;
	//printk("new page:%lx",page_add);
	...
	default:
		...
		//printk("gpa=%lx",(unsigned long)ssa_gpa);
		        asm volatile("vmread %%rax,%0\n\t":"=b"(eptpx):"a"(EPT_POINTER));
		//printk("eptp=%lx",eptpx);
		        eptp = eptpx;
	                ssa_gpa = (unsigned long)ssa_gpa;
	                hpa=0;
		        hpa=get_host_phys_addr(ssa_gpa,eptp);
		        printk("eptp=%lx,gpa=%lx,hpa=%lx",(unsigned long)eptp,(unsigned long)ssa_gpa,(unsigned long)hpa);
...
static void __get_regs(struct kvm_vcpu *vcpu, struct kvm_regs *regs)
{
	...
	//gai4
	//printk("rip=%lx",(unsigned long)regs->rip);
}
				      ...
				      u32 size)
{
	...
	for (i = 0; i < KVM_ADDRESS_SPACE_NUM; i++) {
		...
		//printk("hva:%lx",hva);
		//printk("gpa:%lx",(unsigned long)gpa);
		/*gai4*/
		m.memory_size = size;
