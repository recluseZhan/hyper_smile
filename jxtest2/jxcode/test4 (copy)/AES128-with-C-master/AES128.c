...
{	
    ...
    int numOfPadding = ctlen - ptLen;

    //padding 
    uint8_t ptWithPadding[ctlen];
    PKCS7Padding(pt, ptLen, ptWithPadding, ctlen, numOfPadding);
    printHex(ptWithPadding, ctlen, "plain data with padding in hex:");
   ...
   for (int i = 0; i < ctlen; i += BLOCKSIZE) {
...
{
	if (NULL == key || NULL == ct || NULL == pt || NULL == iv){
        printf("param err.\n");
        return -1;
    }
...
int getCiphertextLen(uint8_t *plainText, int plainTextLen)
{
    ...
    return findTrueLen(CiphertextLen);
}

//Fill the plain text with PKCS7 
void PKCS7Padding(const uint8_t *input, int inputLen, uint8_t * plaintext, int CiphertextLen, int numOfPadding)
{
	int i = 0;
	for(;i < inputLen; i++) 
	{
    ...
    for(;i < CiphertextLen;i++) 
    {
    	plaintext[i] = (uint8_t)numOfPadding;	
    }
}
